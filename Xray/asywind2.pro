pro asywind2, n=n, lout=l, ld=ld, lvr=lvr, lion=lion, time=time, dth=dth,sample=sample
...
if not keyword_Set(sample) then sample=maxref-8
d=dload(n,var='dens',sample=sample,xc=0,yc=0,zc=0,x,y,z,time)
if arg_present(lvr) then begin
  vx=dload(n,var='velx',sample=sample,xc=0,yc=0,zc=0)
  vy=dload(n,var='vely',sample=sample,xc=0,yc=0,zc=0)
endif
...
if not keyword_set(dth) then dth=0.
th+=dth
