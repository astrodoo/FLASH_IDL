pro playjet_guitar,fname,var=var,sample=sample,startn=startn,endn=endn,step=step,png=png,xrange=xrange,yrange=yrange
device, decomposed=0

;if not keyword_set(fname) then fname='jet_xrbs_hdf5_plt_cnt_'
...
;    d = loaddata(fnames[j],var,xra=xrange,time=time,sample=smp,xcoords=x,ycoords=y,zcoords=z)
    d = loaddata(fnames[j],var,time=time,sample=sample,xcoords=x,ycoords=y)
    sd = size(d,/dimension)
...
;    imgxy = reform(d[*,*,z0ind])
    ...
    tv,bytscl(alog(d),min=alog(mind),max=alog(maxd))
